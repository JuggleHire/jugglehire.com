<template>
  <div class="bg-white sm:pt-0 pb-12 lg:py-20 space-y-16">
    <!-- Feature section -->
    <div class="max-w-[85rem] mx-auto space-y-16">
      <div class="mx-auto px-6 lg:px-8">
        <div class="mx-auto max-w-2xl sm:text-center" data-aos="fade-in" data-aos-once="true">
          <h2 class="mt-2 text-2xl !leading-[2.5rem] font-bold tracking-tight text-gray-900 sm:text-4xl sm:!leading-[3.8rem]">
            Single Dashboard. <br> <span class="text-blue-600">Manage Everything.</span>
          </h2>
          <p class="mt-6 text-lg leading-8 text-gray-600">
            🚀 Skillfully Hire The Right Candidate Using World-Class Recruitment
            Software
          </p>
        </div>
      </div>
      <div class="mx-auto px-6 lg:px-8" data-aos="zoom-in" data-aos-once="true">
        <img src="/jugglehire-app.jpg" alt="App screenshot"
          class="mb-[-12%] hover:scale-105 transition-all duration-200 ease-linear rounded-xl " width="2432"
          height="1442" />
      </div>
    </div>
  </div>
</template>

<script setup>
</script>
